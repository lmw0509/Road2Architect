...
内部类是外部类的一个成员，因此**内部类可以自由地访问外部类的成员变量，无论是否是private的。**
...
>1 只是为了降低包的深度，方便类的使用，静态内部类适用于包含类当中，但又不依赖外在的类。
> 
...
>1 成员内部类之所以叫做成员就是说他是类**实例的一部分而不是类的一部分** 
...
> 1  内部类是延时加载的，也就是说只会在第一次使用时加载。不使用就不加载，所以**可以很好的实现单例模式。**
> 
